...
class Epics:
    def __init__(self, cam_dat_eps, initial_ctr_param_values, init_dict=None):
        ...
        self.ao_roi_x_start = builder.aOut(control_param_names['roi_x_start'],
                            ...
                            on_update_name=lambda v, n: self.on_update_data_a('roi', 'roi_x_start', v),
                            always_update=True)
    ...
    def load_names(self, init_dict=None):
        ...
        if init_dict:
            ...
            if ('control_params' in init_dict) and (len(init_dict['control_params']) == 8):
                control_param_names = init_dict['control_params']
            else:
                control_param_names = default_control_param_names

            if ('fit_params' in init_dict) and (len(init_dict['fit_params']) == 7):
                fit_param_names = init_dict['fit_params']
            else:
    ...
    def on_update_data_a(self, area, ctr_param_name, value):
        self.cam_dat_eps.on_update(area, ctr_param_name, value)
