...
class Epics:
    def __init__(self, cam_dat_eps, initial_ctr_param_values, init_dict=None):
        device_name, control_param_names, fit_param_names, error_name = self.load_names(init_dict)
       ...
       #fit parameter
        ...
        #error
        self.ai_error = builder.aIn(error_name, initial_value=0)
    ...
    def set_error(self):
        error_message = self.cam_dat_eps.get_error()
        if error_message:
            self.ai_error.set(error_message)
