...
class GaussianFit:
    ...
    def update(self, fit_area):
        try:
            ...
            # sample values if sampeled is active, inactive: self.sampled = 1
            self.sample_data()
    ...
    def sample_data(self):
        # sample data so every self.sampeled-th element is chosen
            # TO DO: sampled error messages are not leading to a failed analyzation,
            # so they are not yet visible in epics
        if type(self.sampled) == int:
            if self.sampled > 1:
                if self.sampled < len(self.z_values_in):
                    ...
                    error_message = 'sampled parameter can not be bigger than flatted data length'
                    self.send_error(error_message)
            elif self.sampled < 1:
                error_message = 'sampled parameter must be an integer bigger than 0'
                self.send_error(error_message)
        else:
            error_message = 'sampled parameter must be type int'
            self.send_error(error_message)
...
if __name__ == '__main__':
    ...
    class fake_CamDatEps:
        def __init__(self):
            self.ia = ImageAcquirerFile(self, 'D:\\HZB\\Camera_Data\\mls13\\', 200)
            #self.ia = ImageAcquirerVimba(fake_CamDatEps, {"features": {"ExposureAuto":"Off"}})
